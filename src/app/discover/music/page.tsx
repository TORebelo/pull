...
export default async function DiscoverMusicPage() {
    const items = await prisma.profileItem.findMany({
        where: { type: "MUSIC", visibility: "PUBLIC" },
    ...
    return (
        <section className="grid gap-3">
            ...
            {items.length === 0 ? (
                <div className="rounded-2xl border border-amber-950/10 bg-[var(--color-surface)] p-4 text-sm text-[var(--color-ink-soft)]">No public music shares yet.</div>
            ) : null}
        </section>
    );
}
